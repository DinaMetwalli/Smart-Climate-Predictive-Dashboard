"""
A multi-step model that takes in multiple continents and produces a prediction for each.

Note: produces forecast_num months (multiple months) for each prediction.
Does not have monthly encoding as input features.
"""
...
class MultiContinentForecastingModel():
    ...
    def __init__(self, regions=None, seq_len:int = 36, forecast_num:int = 12, model_file: str = None, scaler_file: str = None):
        ...
        self.model = MultiOutputLSTM(
            ...
            forecast_num=self.forecast_num
        )
    ...
    def save_model(self, path:str = "multi_continent_lstm.pth") -> None:
        ...
        print(f"Scalers saved to 'my_scalers.pkl'.")
    ...
    def __encode_features(self, wide_df, fit=True, start_idx=0) -> np.ndarray:
        """
        ...
        # df['Month_Idx'] = np.arange(start_idx, start_idx + len(df)) % 12
    ...
    def __create_sequences(self, data):
        ...
        for i in range(len(data) - self.seq_len - self.forecast_num + 1):
            # Input: seq_len timesteps with all features
            X.append(data[i : i + self.seq_len])
            
            # Target: forecast_num timesteps for regions only
            y.append(data[i + self.seq_len : i + self.seq_len + self.forecast_num, :self.num_regions])
            
            # Future time features: we need the future sin/cos to feed into the autoregressive loop
    ...
    def train_model(self):
        ...
        split_idx = int(len(wide_df) * TRAIN_SPLIT)

        train_data = self.__encode_features(wide_df.iloc[:split_idx], fit=True, start_idx=0)
        test_data  = self.__encode_features(wide_df.iloc[split_idx:], fit=False, start_idx=split_idx)
        ...
        self.model.eval()
        with torch.no_grad():
            train_pred_full = self.model(X_train, future_steps=self.forecast_num, future_time_features=time_train)
            test_pred_full  = self.model(X_test,  future_steps=self.forecast_num, future_time_features=time_test)
        ...
        self.__print_summary_stats(y_train.reshape(-1, self.num_regions), train_pred_full.reshape(-1, self.num_regions), dataset_name="Training")
        self.__print_summary_stats(y_test.reshape(-1, self.num_regions), test_pred_full.reshape(-1, self.num_regions), dataset_name="Validation")
    ...
    def __plot_training_results(self, X_train, X_test, time_train, time_test, split_idx, wide_df):
        ...
        # Only use first forecast step for alignment across the main plot sequence
    ...
    def predict_future(self, dataset:pd.DataFrame, months_to_test:int = 48, test_future:bool = True):
        ...
        if not test_future:
            predict_start_idx = total_len - 200
        ...
        if not test_future:
            ...
            self.__print_summary_stats(actual, pred, dataset_name="Backtest")
...
class MultiOutputLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim, num_regions, forecast_num):
        super().__init__()
        ...
        # Predict all steps for all regions in one shot
        self.fc = nn.Linear(hidden_dim, forecast_num * num_regions)

    def forward(self, x, **kwargs):
        out, _ = self.lstm(x)
        last_hidden = self.dropout(out[:, -1, :])
        flat = self.fc(last_hidden)
        return flat.view(-1, self.forecast_num, self.num_regions)