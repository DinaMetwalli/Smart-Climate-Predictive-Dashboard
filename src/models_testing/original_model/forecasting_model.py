"""
Original working forecasting model which takes in the global dataframe with all continents
and predicts a single continent in a one-shot prediction.

Left unmodified, only added performance stats print function.
"""
...
class ClimateForecastingModel():
    ...
    def train_model(self):
        ...
        optimizer = torch.optim.Adam(self.model.parameters(), lr=0.001) #0.005
        loader = data.DataLoader(data.TensorDataset(X_train, y_train), shuffle=False, batch_size=128) #256
    ...
    def __plot_model_loss(self, train_rmse_values, val_rmse_values) -> None:
        ...
        epochs = np.arange(0, len(train_rmse_values) * 5, 5)
    ...
    def predict_future(self, dataset:pd.DataFrame, months_to_test:int = 48, test_future:bool = True):
        ...
        if not test_future:
            actuals = dataset['Anomaly'].values[predict_start_idx : predict_start_idx + steps_to_predict]
            self.__print_summary_stats(actuals, blind_preds_rescaled.flatten(), "Backtest")
        # return preds
